...
export default function AddServerPage() {
  ...
  // Widget status hook
  const { status: widgetStatus, isChecking: isCheckingWidget, errorMessage: widgetError } = useWidgetStatus(widgetId);
  ...
  if (!session) return (
    <main className="flex flex-col items-center justify-start min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 dark:from-gray-900 dark:to-black transition-colors pt-10 sm:pt-20">
      <div className="w-full max-w-md bg-gray-900/80 rounded-2xl shadow-xl p-4 sm:p-8 border border-gray-800 animate-fade-in text-center mx-auto mt-4 sm:mt-12 mb-8 sm:mb-0">
        ...
        <p className="mb-6 text-zinc-400 text-sm sm:text-base">You must be logged in to access the server listing feature and manage your Discord servers.</p>
        <a 
          href="/profile" 
      ...
      <div className="w-full max-w-2xl bg-gray-900/80 rounded-2xl shadow-xl p-2 sm:p-6 md:p-8 border border-gray-800 animate-fade-in">
        ...
        {botInvited && (
          <>
            ...
            <div className="mb-4 sm:mb-6">
              <label className="block mb-1 sm:mb-2 font-semibold text-gray-200 text-sm sm:text-base">
                Discord Widget Server ID (optional):
              </label>
              <div className="relative">
                <input
                  type="text"
                  ...
                  className={`block w-full p-2 sm:p-3 bg-gray-800 text-white border rounded-lg sm:rounded-xl focus:ring-2 focus:outline-none mb-1 sm:mb-2 text-sm sm:text-base pr-10 ${
                ...
                <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center">
                  ...
                  {!isCheckingWidget && widgetStatus === 'enabled' && (
                    <div className="text-green-500" title="Widget is enabled">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                      </svg>
                    </div>
                  )}
                  {!isCheckingWidget && widgetStatus === 'disabled' && (
                    <div className="text-red-500" title="Widget is disabled">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                      </svg>
                    </div>
                  )}
                  {!isCheckingWidget && widgetStatus === 'error' && (
                    <div className="text-yellow-500" title={widgetError || 'Invalid server ID'}>
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              ...
              {widgetId && widgetStatus !== 'loading' && (
                <div className={`text-xs mt-1 ${
                  ...
                  {widgetStatus === 'enabled' && '✓ Widget is enabled and working'}
                  {widgetStatus === 'disabled' && '✗ Widget is disabled - Enable in Discord: Server Settings → Widget'}
