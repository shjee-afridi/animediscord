...
async function getServerData(guildId: string) {
  try {
    ...
    const baseUrl = process.env.NEXTAUTH_URL || 'https://animediscord.com';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!server) {
    return {
      title: 'Server Not Found - Anime Discord',
      description: 'The requested Discord server could not be found.',
    };
  }

  const title = `${server.name} - Discord Server | Anime Discord`;
  ...
  const serverIconUrl = server.icon 
    ...
    : 'https://animediscord.com/icon-512x512.png'; // Use full URL for fallback
  
  const serverUrl = `${process.env.NEXTAUTH_URL || 'https://animediscord.com'}/server/${params.guildId}`;
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "url": `${process.env.NEXTAUTH_URL || 'https://animediscord.com'}/server/${server.guildId}`,
              "image": server.icon 
                ? `https://cdn.discordapp.com/icons/${server.guildId}/${server.icon}.png?size=512`
                : 'https://animediscord.com/icon-512x512.png',
              "isPartOf": {
                "@type": "WebSite",
                "name": "Anime Discord",
              ...
              "about": {
                ...
                "image": server.icon 
                  ? `https://cdn.discordapp.com/icons/${server.guildId}/${server.icon}.png?size=512`
                  : 'https://animediscord.com/icon-512x512.png',
                "url": server.link || `${process.env.NEXTAUTH_URL || 'https://animediscord.com'}/server/${server.guildId}`,
